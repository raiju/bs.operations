...
all_file_params = [{'id': x, 'type': 'file', 'multiple': x[-5:] == "files"} for x in all_params if (x[-4:] == 'file' or x[-5:] == 'files')]
other_params = [
    {'id': 'tool', 'type': 'list'},
...
class BedToolsForm(base.BaseForm):
    ...
    for it in file_params['multiple']:
        vars()[it] = base.MultipleFileUpload(label=it + ': ', validator=twf.FileValidator())
...
class BedToolsPlugin(base.OperationPlugin):
